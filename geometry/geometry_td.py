...
class GeometryWithTime(Geometry):
    ...
    def _sampling_domain_samples(self):
        ...
        if self.sampling_config.time.random_sampling:
            ...
            domain_points = np.repeat(domain_points, len(time_points), axis=0) # repeat each element len(time_points) times
            time_points = np.tile(time_points, (length, 1)) # repeat time_points array for length times
